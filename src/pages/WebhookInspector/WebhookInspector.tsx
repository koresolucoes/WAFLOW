...
const WebhookInspector: React.FC = () => {
    ...
    return (
        <div className="space-y-8 h-full flex flex-col">
            <h1 className="text-3xl font-bold text-white flex-shrink-0">Webhook Inspector</h1>
            <Card className="flex-grow overflow-hidden flex flex-col">
                <p className="text-slate-400 text-sm mb-4 flex-shrink-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-grow overflow-hidden">
                    {/* Log List */}
                    <div className="md:col-span-1 bg-slate-900/50 p-2 rounded-lg overflow-y-auto">
                        {isLoading ? (
                            <p className="text-center text-slate-400 p-4">Carregando...</p>
                        ) : logs.length === 0 ? (
                            <div className="flex items-center justify-center h-full">
                                <p className="text-center text-slate-400 p-4">Nenhum evento de webhook recebido ainda.</p>
                            </div>
                        ) : (
                            <ul>
                                {logs.map(log => (
                                    <li key={log.id}>
                                        <button
                                            ...
                                            className={`w-full text-left p-3 rounded-md mb-2 transition-colors ${selectedLog?.id === log.id ? 'bg-sky-500/20' : 'hover:bg-slate-700/50'}`}
                                        >
                                            <div className="flex justify-between items-center text-xs text-slate-400">
                                                <span className={`font-semibold ${log.source === 'meta_message' ? 'text-green-400' : 'text-purple-400'}`}>{log.source}</span>
                                                <span>{new Date(log.created_at).toLocaleTimeString('pt-BR')}</span>
                                            </div>
                                            <p className="font-mono text-sm text-slate-200 mt-1 truncate">{log.path}</p>
                                        </button>
                                    </li>
                                ))}
                            </ul>
                    ...
                    <div className="md:col-span-2 bg-slate-900/50 p-4 rounded-lg overflow-y-auto">
                         <h3 className="text-lg font-semibold text-white mb-2">Payload</h3>
                        {selectedLog ? (
